...
void AShooterProjectile::NotifyHit(class UPrimitiveComponent* MyComp, AActor* Other,
                                   ...
                                   FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	{
		GetWorld()->GetTimerManager().SetTimer(DestructionTimer, this, &AShooterProjectile::OnDeferredDestruction,
...
void AShooterProjectile::ExplosionCheck(const FVector& ExplosionCenter)
{
	...
	GetWorld()->OverlapMultiByObjectType(Overlaps, ExplosionCenter, FQuat::Identity, ObjectParams, OverlapShape,
	                                     QueryParams);
	...
	for (const FOverlapResult& CurrentOverlap : Overlaps)
	{
		...
		if (DamagedActors.Find(CurrentOverlap.GetActor()) == INDEX_NONE)
		{
			...
			ProcessHit(CurrentOverlap.GetActor(), CurrentOverlap.GetComponent(), GetActorLocation(),
                                    ...
                                    const FVector& HitDirection)
{
	...
	if (ACharacter* HitCharacter = Cast<ACharacter>(HitActor))
	{
		...
		if (HitCharacter != GetOwner() || bDamageOwner)
		{
			...
			UGameplayStatics::ApplyDamage(HitCharacter, HitDamage, GetInstigator()->GetController(), this,
			                              HitDamageType);
		}
	}
